...
import (
   ...
   "bytes"
   "strconv"
   log "github.com/watsonjiang/crowndb/log"
)
...
func (m *mempool_t) init_allocators(factor float32) {
   ...
   size := ITEM_HEAD_SIZE + ITEM_PAYLOAD_MIN  //init item size
...
/*
Allocate Item with len(key)=nkey and len(value)=nval
*/
...
func (m *mempool_t) ItemRealloc(it Item, new_nval int) Item {
   old_it := it.(*item_t)
   old_nsize := item_size(old_it.nkey, old_it.nval)
   old_idx := m.allocator_idx(old_nsize)
   new_nsize := item_size(size_t(old_it.nkey), size_t(new_nval))
   ...
   new_it.nval = size_t(new_nval)
   //TODO: copy the data field 
   m.allocators[old_idx].free_item(old_it)
   return new_it
}
